...
function Location() {
  ...
  const tollguruUrl = 'http://localhost:3001/tollguru/toll/v2/complete-polyline-from-mapping-service'
  ...
  const fetchData = async () => {
    ...
    try {
      console.log('City:',cityFuel,'Highway',highwayFuel,'Fuel:',fuelPrice);
      ...
      setStrtCoordinates(data.resourceSets[0].resources[0].routeLegs[0].actualStart.coordinates);
      setEndCoordinates(data.resourceSets[0].resources[0].routeLegs[0].actualEnd.coordinates)

      // Extract polyline coordinates from the response
      const temp = data.resourceSets[0].resources[0].routePath.line.coordinates;
      setTempData(temp);
      ...
      const datas = {
        ...
        fuelOptions: {
          fuelCost: { value: fuelPrice, units: 'INR/Liter', currency: 'INR', fuelUnit: 'Liter' },
      ...
      console.log('Tolls Data from', source, 'to', destination, ': ', 'Cash:',
  ...
  return (
    <mapData.Provider value={wholeData}>
      <div className='lcn'>
        ...
        <div className='onTop'>
          {/* <form> */}
            <input type='text' placeholder='Enter Starting Location' onChange={(e) => setSource(e.target.value)} required></input>
            <input type='text' placeholder='Enter Destination' onChange={(e) => setDestination(e.target.value)} required></input>
            <button onClick={fetchData}>Check</button><br />
          {/* </form> */}
          <b><span style={{ color: "black" }}>Select your vehicle</span></b>
          <select style={{ borderRadius: '20px' }} onChange={vehicleData}>
            ...
            <option value={"2AxlesLCV"}>Pickup Truck, Light Commercial Vehicle</option>
          ...
          {showFuelInputs && (
            <><br/>
              <label for='city'>City Milage:</label><input type='number' id='City' value={cityFuel} onChange={(e) => setCityFuel(e.target.value)} /><br/>
              <label for='Highway'>Highway Milage:</label><input type='number' id='Highway' value={highwayFuel} onChange={(e) => setHighwayFuel(e.target.value)} /><br/>
              <label for='Fuel'>Fuel Price:</label><input type='number' id='Fuel' value={fuelPrice} onChange={(e) => setFuelPrice(e.target.value)} />
            </>
          )}

          {tempData && <div><b>Total Toll (cash) :</b> {cash}</div>}
          {tempData && <div><b>Distance:</b> {distance}</div>}
          {tempData && <div><b>Fuel Cost:</b> {fuel}</div>}
        </div>
      </div>
    </mapData.Provider>
